  ...
  try {
    ...
    try {
      ...
      if (queryResult.serverResponse?.fileList && queryResult.serverResponse.fileList.length > 0) {
        ...
        console.log('File information found:', fileInfo);
      ...
      // but we can't query it anymore. In this case, we'll return the current status.
      return NextResponse.json({
        success: true,
        status: recording.status,
        message: 'Unable to query recording status from Agora (this is normal after recording completion)',
        info: 'The recording may be completed but no longer queryable. Check your S3 bucket directly.'
      });
    }
